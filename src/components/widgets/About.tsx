...
import { AboutProps, ContentProps } from '~/shared/types';
    ...
    <div className="mx-auto max-w-7xl">
      <div className={`md:flex ${isReversed ? 'md:flex-row-reverse' : ''} md:gap-16`}>
        <div className="self-center md:basis-3/5">
          ...
          {moreContent && <div className="mb-8 lg:mb-12 text-lg text-gray-600 dark:text-slate-400">{moreContent}</div>}
          {evenMoreContent && <div className="mb-8 lg:mb-12 text-lg text-gray-600 dark:text-slate-400">{evenMoreContent}</div>}
          <br /><br /><br />
       {/* <ItemGrid
          items={items}
          columns={1}
          defaultIcon={IconCheck}
          containerClass="w-full gap-4 md:gap-y-10" // Ensure the container is full width
          panelClass="flex w-full" // Ensure each panel spans full width
      ...
      <ItemGrid
          ...
          containerClass="w-full gap-4 md:gap-y-10" // Ensure the container is full width
          panelClass="flex w-full" // Ensure each panel spans full width
          titleClass="text-2xl font-bold leading-6 text-gray-900 dark:text-white mt-1 mb-2"
          descriptionClass="mt-1 font-bold text-xl text-blue-800 dark:text-slate-400"
          iconClass="flex-shrink-0 w-7 h-7 flex items-center justify-center rounded-full bg-primary-900 text-gray-50 mr-4 rtl:mr-0 rtl:ml-4 mt-1 p-1"
        />
      </div>
    </div>
